...
**AgentPipe** intercepts requests from **any OpenAI-compatible coding agent** (such as **Cline**, **Roo Code**, **Continue**, **Aider**, or custom agentic workflows) and displays them on a sleek local web dashboard. You copy the prompt to your favorite web chatbot (**ChatGPT**, **Claude**, **Gemini**, **DeepSeek**) and paste the response back. AgentPipe streams it directly to your agent via standard OpenAI Server-Sent Events (SSE).
...
| **API Key** | `agentpipe` *(or any arbitrary string)* |
...
```
[ Any Coding Agent ] ──(Request)──> [ AgentPipe :4747 ] ──(Web Dashboard)
                                                                    │
                                                            (Copy / Paste)
                                                                    ▼
[ Any Coding Agent ] <──(SSE Stream)── [ AgentPipe ] <── [ Web Chatbot ]
```
...
3. **Dispatch:** Paste the LLM's response into the dashboard and press `Ctrl + Enter` (or click **Send**). AgentPipe handles token streaming, JSON sanitization, and execution automatically.
